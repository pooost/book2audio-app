...
def extract_with_review(
    ...
    page's OCR text is checked against its own source image before being
    concatenated. AI review only makes sense against an actual OCR
    transcription -- content whose real text layer is used directly (no
    OCR involved) skips review, with ExtractResult.ai_review_skip_reason
    explaining why. Falls back to the simpler extract_markdown() path
    whenever review isn't requested, unchanged from before this existed.
    """
    suffix = input_path.suffix.lower()
    is_image_input = suffix in IMAGE_SUFFIXES or input_path.is_dir()
    ...
    if not uses_ocr:
        text = markitdown_text if markitdown_text is not None else run_markitdown(input_path)
        skip_reason = "This document's text layer was used directly -- no OCR transcription to review." if ai_review else None
        return ExtractResult(text=text, ai_review_skip_reason=skip_reason)

    if not ai_review:
        return ExtractResult(text=_ocr(input_path, ocr_output_dir, allow_download))
...
def _ocr_with_review(
    ...
    try:
        for i, img_path in enumerate(page_images, start=1):
            ...
            result = review_page(img_path, page_texts[img_path], model=ai_review_model)
        ...
        # load Chatterbox -- see unload_model()'s docstring for why this
    ...
    return ExtractResult(text="\n\n".join(reviewed_texts), review_flags=flags, ai_review_applied=True)
